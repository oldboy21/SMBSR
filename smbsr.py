        ...
        try:
            ...
            if len(results) == 0:
                ...
                cursor.execute(insertFindingQuery, (filename, share, ip, line,
                               matchedwith, times[0], times[1], times[2], date, date, tag, text))
                self.commit()
            else:
                textOld = ((results[0])[1])
        ...
        try:
            ...
            if len(results) == 0:
                ...
                cursor.execute(insertFindingQuery, (filename, share,
                               ip, times[0], times[1], times[2], date, date, tag))
                self.commit()
            else:
...
class HW(object):
    def __init__(self, workername, options, db):
        ...
        self.conn = SMBConnection(options.username, options.password, options.fake_hostname,
    ...
    def dnsLookup(self, target):
        try:
            ...
            logger.warning(
                f"[{self.workername}] exception reading from initial list")

        if not valid:
            logger.info(
                f"[{self.workername}] You entered an hostname, looking up " + i)
            try:
                ...
                logger.warning(
                    f"[{self.workername}] Hostname could not be resolved: " + i)
    ...
    def retrieveTimes(self, share, filename):
        try:
            ...
            ts_created = datetime.fromtimestamp(
                attributes.create_time).strftime('%Y-%m-%d %H:%M:%S')
            ts_accessed = datetime.fromtimestamp(
                attributes.last_access_time).strftime('%Y-%m-%d %H:%M:%S')
            ts_modified = datetime.fromtimestamp(
                attributes.last_write_time).strftime('%Y-%m-%d %H:%M:%S')
            times.append(ts_created)
            times.append(ts_modified)
            times.append(ts_accessed)
            return times
        except Exception as e:
            logger.error("Error while retrieving timestamp of file: " +
            ...
            if output:
                try:
                    ...
                    for z in m:
                        ...
                        self.db.insertFinding(filename, share, IP, str(counter), words[z], self.retrieveTimes(
                            share, filename), self.options.tag, tosave.replace("\n", " "))
                        return True
                except Exception as e:
                    logger.debug(
                    ...
                    try:
                        ...
                        if matchedraw:
                            ...
                            if len(text) < 50:
                                ...
                                tosave = text[lbound:ubound]
                            self.db.insertFinding(filename, share, IP, str(counter), i, self.retrieveTimes(
                                share, filename), self.options.tag, tosave.replace("\n", " "))
                            return True
                    except Exception as e:
                        logger.debug(
    ...
    def parse(self, share, filename, to_match, IP):
        ...
        if file_ext.lower() in self.options.file_extensions_black.split(','):
            ...
            if file_ext.lower() in self.options.file_interesting.split(','):
                ...
                self.db.insertFileFinding(filename, share, IP, self.retrieveTimes(
                    share, filename), self.options.tag)
            ...
            if filesize > self.options.max_size:
                ...
                if file_ext.lower() in ['docx', 'doc', 'docx', 'eml', 'epub', 'gif', 'jpg', 'mp3', 'msg', 'odt', 'ogg', 'pdf', 'png', 'pptx', 'ps', 'rtf', 'tiff', 'tif', 'wav', 'xlsx', 'xls']:
                    specialfile = open(str(''.join(random.choices(
                        string.ascii_uppercase, k=5))) + "." + file_ext, "ab")
                    file_attributes, filesize = self.conn.retrieveFile(
                        share, filename, specialfile)
                    ...
                    if lines != "textractfailed":
                        ...
                        try:
                            ...
                            logger.error(
                                f"[{self.workername}] Error deleting the temp file: " + specialfile.name)
                ...
                if p.filename != '.' and p.filename != '..':
                    ...
                    if p.isDirectory:
                        ...
                        if p.filename.lower() in list(map(lambda x: x.lower(), self.options.folder_black.split(','))):
                            logger.debug(
                                f"[{self.workername}] Skipping " + p.filename + " since blacklisted")
                            ...
                            if parentPath.count('/') <= self.options.depth:
                                ...
                                logger.debug(
                                    f"[{self.workername}] Visiting subfolder " + str(p.filename))
                                try:
                                    count = count + \
                                        self.walk_path(
                                            parentPath+p.filename, shared_folder, IP, to_match)
                                except Exception as e:
                                    logger.error(
                                        f"[{self.workername}] Error while accessing folder: " + parentPath+p.filename)
                                    continue
                                # IF IT FAILS WITH A FOLDER IT SHOULD TRY TO MOVE FORWARD
                            else:
                                logger.debug(
                                    f"[{self.workername}] Skipping " + str(parentPath+p.filename) + ", too deep")
                                continue
                    else:
                        logger.debug(
                            f"[{self.workername}] File: " + parentPath+p.filename)
                        self.parse(shared_folder, parentPath +
                                   p.filename, to_match, IP)

            return count
        except Exception as e:
            logger.error(
                f"[{self.workername}] Error while listing path: "+path+" in share: "+shared_folder)
    ...
    def shareAnalyze(self, IPaddress, to_match):
        for ip in IPaddress:
            ...
            if self.options.uncpaths:
                ...
                logger.debug(
                    f"[{self.workername}] Connecting to: {host} on share {share} with startdir {start_dir}")
                try:
                    self.conn.connect(host, 445)
                except Exception as e:
                    logger.error(f"[{self.workername}] Detected error while connecting to " + str(
                        target) + " with message " + str(e))
                    continue
            ...
            try:
                ...
                logger.error(
                    f"[{self.workername}] Detected error while connecting to " + str(ip) + " with message " + str(e))
                continue
            try:
                shares = self.conn.listShares()

            except Exception as e:
                logger.error(f"[{self.workername}] Detected error while listing shares on " + str(
                    ip) + " with message " + str(e))
                continue
            for share in shares:
                if not share.isSpecial and share.name not in ['NETLOGON', 'IPC$'] and share.name not in self.options.share_black.split(','):
                    logger.debug(
                        f"[{self.workername}] Listing file in share: " + share.name)
                    try:
    ...
    def scanNetwork(self):
        ...
        if self.options.ldap:
            logger.info(
                "Retrieving computer objects from LDAP. KINIT process might take some time, be patient")
        ...
        if not self.options.masscan:
            if len(final) == 0 and len(cidrs) > 0:  # case only one input is given and it is a CIDR
                logger.error(
                    "Hey there, if you do not use masscan you can't give me CIDR as input")
                sys.exit(1)
        ...
        if len(cidrs) > 0:
            for ni in cidrs:
                try:
                    ...
                    logger.error("masscan failed with error: " +
                                 str(e) + " for range: " + str(ni))
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        add_help=True, description="SMB Password Revealer ")
    ...
    parser.add_argument('-domain', action='store',
                        default='SECRET.LOCAL', help='Domain for authenticated scan')
    parser.add_argument('-fake-hostname', action='store', default='localhost',
                        help='Computer hostname SMB connection will be from')
    parser.add_argument('-word-list-path', action="store", default='unset',
                        type=str, help="File containing the string to look for")
    parser.add_argument('-max-size', action="store", default=50000, type=int,
                        help="Maximum size of the file to be considered for scanning (bytes)")
    parser.add_argument('-file-extensions-black', action='store', type=str, default='none',
                        help='Comma separated file extensions to skip while secrets harvesting')
    parser.add_argument('-multithread', action='store_true',
                        default=False, help="Assign a thread to any IP to scan")
    parser.add_argument('-masscan', action='store_true', default=False,
                        help="Scan for 445 before trying to analyze the share")
    parser.add_argument('-T', action='store', default=10,
                        type=int, help="Define the number of thread to use")
    parser.add_argument('-logfile', action='store',
                        default='smbsr.log', type=str, help='Log file name')
    parser.add_argument('-dbfile', action='store',
                        default='./smbsr.db', type=str, help='Log file name')
    parser.add_argument('-file-interesting', action='store', default='none', type=str,
                        help='Comma separated file extensions you want to be notified about')
    parser.add_argument('-folder-black', action='store', default='none', type=str,
                        help='Comma separated folder names to skip during the analysis, keep in mind subfolders are also skipped')
    parser.add_argument('-csv', action='store_true', default=False,
                        help='Export results to CSV files in the project folder')
    parser.add_argument('-depth', action='store', default=100000000, type=int,
                        help='How recursively deep you want to go while looking for secrets')
    parser.add_argument('-regular-exp', action="store", default='unset',
                        type=str, help="File containing regex expression to match")
    parser.add_argument('-share-black', action='store', type=str, default='none',
                        help='Comma separated share names to skip while secrets harvesting')
    parser.add_argument('-uncpaths', action='store_true', default=False,
                        help='Switch to use a UNC path as a starting point')
    group = parser.add_mutually_exclusive_group()
    group.add_argument('-ip-list-path', action="store",
                       default="unset", type=str, help="File containing IP to scan")
    group.add_argument('-IP', action="store",
    ...
    parser.add_argument('-hits', action='store', default=5000,
                        type=int, help='Max findings per file')
    parser.add_argument('-ntlm', action='store_true', default=False,
                        help="Use NTLM authentication for LDAP auth, default is Kerberos")
    parser.add_argument('-tag', action='store',
                        default="NOLABEL", type=str, help='Label the run')
    parser.add_argument('-debug', action='store_true',
                        default=False, help='Verbose logging enabled')

    options = parser.parse_args()

    if options.ldap and not options.fqdn:
        parser.error(
            'If you want to retrieve computer objects from LDAP please provide a FQDN to retrieve your TGT')
    faulthandler.enable()
    ...
    if options.multithread is True:
        ...
        for i in range(options.T):
            try:
                ...
                worker = smbworker("Worker-" + str(i+1),
                                   options, to_analyze, to_match, db)
                worker.start()
